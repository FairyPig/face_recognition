...
def runLBP():
    ...
    judgeImgs = judgeImgs.T
    ImgLBPope = LBP(judgeImgs)
    judgeHistogram = np.mat(np.zeros((256 * 4 * 7, np.shape(ImgLBPope)[1])))
    for i in range(np.shape(ImgLBPope)[1]):
        exHistogram = calHistogram(ImgLBPope[:, i], 7, 4)
        judgeHistogram[:, i] = exHistogram
    judgeHistogram = judgeHistogram.transpose()
    judgeHistogram = judgeHistogram - mean;
    judgeHistogram = np.dot(judgeHistogram, d)
    ...
    for num in range(1,150):
        ...
        clf_probs = clf.predict_proba(judgeHistogram)
        score = log_loss(y_test, clf_probs)
        result = clf.predict(judgeHistogram)
